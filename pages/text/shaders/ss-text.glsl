...
const int numOfRepelers = @REPELERS;
uniform vec3 repelPos[ numOfRepelers ];
...
void main(){
  ...
  float displace =  .00000001 * snoise( (to.xy + offset ) * .1 );
...
//  vec4 rotPos = vec4( to.x , to.y , -5. 
 ...
 for( int i = 0; i < numOfRepelers; i++ ){
    //if( repelPos[i] != vec3( 0. , 0. , 0. )){
      vec3 toFriend = ( repelPos[i] - pos.xyz);
      vel -= .00000001 *  normalize(toFriend) * repelForce / (length( toFriend * toFriend));
    //}
  }
